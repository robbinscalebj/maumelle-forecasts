noaa_gefs_point_download_downscale_WET <- function(read_from_path,#Move down?
                                               lat_list,
                                               lon_list,
      ...
      for(site_index in 1:length(site_list)){
        ...
        if(length(list.files(model_site_date_hour_dir)) != 31){
          ...
          if(model_list[m] %in% model.runs$model.run){
            ...
            cf_var_units <- c("K", "Pa", "percent", "Wm-2", "Wm-2", "kgm-2s-1", "ms-1", "ms-1", "1")
           ...
           # specific_humidity <- rh2qair(rh = noaa_data$relative_humidity$value / 100,
            ...
            # Convert the 6 hr precip rate to per second = 1 kg /m2/s = 1 mm/s
            ...
            for (ens in 1:31) { # i is the ensemble number
              ...
              #Write netCDF
              Rnoaa4cast::write_noaa_gefs_netcdf(df = forecast_noaa_ens,ens, lat = lat_list[site_index], lon = lon_east, cf_units = cf_var_units, output_file = output_file, overwrite = overwrite)
              ...
              if(downscale){
                ...
                #Run downscaling
                temporal_downscale_WET(input_file = output_file, output_file = fname_ds, overwrite = overwrite, hr = 1)
              }
            }
          }
        }else{
